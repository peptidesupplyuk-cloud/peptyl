...
// Peptyl Research Agent — Daily PubMed ingestion via OpenAI
// Deploy: supabase functions deploy research-agent
// Secret required: OPENAI_API_KEY (already set in Supabase secrets)
...
// Peptide/compound search terms for PubMed
const SEARCH_TERMS = [
  "BPC-157 peptide",
...
// OpenAI extraction
// ============================================================
async function extractWithOpenAI(article: any): Promise<any> {
  ...
  for (const term of SEARCH_TERMS) {
    ...
    try {
      ...
      for (const article of articles) {
        ...
        // Extract via OpenAI
        const extracted = await extractWithOpenAI(article);
        ...
        // Insert into queue for admin review
